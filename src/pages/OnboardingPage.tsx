...
export default function OnboardingPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Couldn't create the kitchen");
      setBusy(false);
    }
  }
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[430px] flex-col justify-center px-6">
      <h1 className="font-display text-[2rem] leading-tight text-ink">Create your kitchen</h1>
      <p className="mt-2 text-ink-muted">
        One kitchen for this house. You’ll tap your name to sign in — no email, no password.
      </p>
      ...
      <form onSubmit={submit} className="mt-6 space-y-3">
        ...
        <Button type="submit" full disabled={busy}>
          {busy ? "Just a moment…" : "Get started"}
        </Button>
      </form>
    </div>
  );
