...
export default function LibraryPage() {
  ...
  async function saveEdit() {
    ...
    await updateRecipe(editing.id, {
      title_id: draft.title_id.trim() || editing.title_id,
  ...
  return (
    <div className="px-4 pb-8 pt-5">
      <header className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h1 className="font-display text-[1.9rem] leading-none text-ink">Koleksi</h1>
          <p className="mt-1 text-sm text-ink-muted">
            {recipes.length} resep · {recipes.filter((r) => r.verdict === "keeper").length} disimpan
          </p>
        </div>
        <Button onClick={() => setAdding(true)}>
          <Icon name="plus" size={18} /> Resep
        </Button>
      </header>
            ...
            >
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                ...
                <p className="mt-0.5 text-sm text-ink-muted">
                  {[formatMinutes(recipe.total_minutes), recipe.tags.slice(0, 3).join(" · ")]
                    .filter(Boolean)
                    .join(" · ") || "—"}
                </p>
        ...
        <div className="rounded-card border border-dashed border-paper-line px-4 py-14 text-center">
          ...
          {!recipes.length && (
            <Button className="mt-4" onClick={() => setAdding(true)}>
              <Icon name="plus" size={18} /> Tambah resep pertama
            </Button>
          )}
        </div>
      )}
...
function PlanSheet({
  ...
  async function plan(slot: Slot) {
    ...
    try {
      ...
      setDone(`${recipe.title_id} masuk ${SLOT_LABEL[slot].toLowerCase()} ${longDate(date)}.`);
    } catch (err) {
      setDone(err instanceof Error ? err.message : "Gagal merencanakan");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Sheet open onClose={onClose} title="Rencanakan" subtitle={recipe.title_id}>
      {done ? (
        ...
        <div className="space-y-2">
          {(upcoming.length ? upcoming : days).map((d) => (
            ...
            >
              <span className="text-ink">{longDate(d)}</span>
